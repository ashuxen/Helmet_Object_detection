...
class SingleInference_YOLOV7:
    ...
    @article{wang2022yolov7,
     ...
     Steven Smiley :  https://github.com/stevensmiley1989/Full_Loop_YOLO by Steven Smiley 2022/11/24
    '''
    ...
    def inference(self):
        ...
        if type(self.im) != type(None):
            ...
            # Apply NMS
            self.outputs = self.non_max_suppression(self.outputs, self.conf_thres, self.iou_thres, classes=None,
                                                    agnostic=False)
            img_i = self.im0.copy()
            ...
            for i, det in enumerate(self.outputs):
                if len(det):
                    ...
                    for j, (*bboxes, score, cls_id) in enumerate(reversed(det)):
                        ...
                        cv2.putText(image, name, (self.box[0], self.box[1] - 2), cv2.FONT_HERSHEY_SIMPLEX, 0.75,
...
if __name__ == '__main__':
    ...
    path_img_i = r"test_images/hard_hat_workers76.png"

    # INITIALIZE THE app
    app = SingleInference_YOLOV7(img_size, path_yolov7_weights, path_img_i, device_i='cpu', conf_thres=0.25,
                                 iou_thres=0.5)

    # LOAD & INFERENCE
    app.load_model()  # Load the yolov7 model
    app.read_img(
        path_img_i)  # read in the jpg image from the full path, note not required if you want to load a cv2matrix instead directly
